  ...
  return (
    <section className="py-10 sm:py-12 lg:py-14 px-4 sm:px-6 lg:px-8 section-glass-tint">
      <div className="container mx-auto max-w-6xl">
        ...
        <div className="relative grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5 items-stretch">
          ...
          {cycleItems.map((item, index) => (
            <div
              key={index}
              className="relative bg-white rounded-2xl p-5 sm:p-6 border border-border shadow-sm hover:shadow-xl transition-all duration-300 hover:-translate-y-1 flex flex-col items-center text-center"
            >
              <div className="relative z-10 w-[72px] h-[72px] rounded-2xl flex items-center justify-center mb-4 shadow-lg flex-shrink-0"
              ...
              <p className="text-[13px] sm:text-sm text-muted-foreground leading-relaxed flex-1">{item.text}</p>
            </div>
          ))}
        </div>
      </div>
